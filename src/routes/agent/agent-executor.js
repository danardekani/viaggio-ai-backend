...
async function executeSearchTours(input) {
  ...
  logger.info(`Searching tours in ${destination}`, {
    interests,
    sort_by,
    start_date,
    end_date,
  ...
  try {
    ...
    try {
      ...
      if (autocompleteResults && autocompleteResults.length > 0) {
        ...
        logger.info(
          `Autocomplete matched "${destination}" -> "${searchDestination}" (ID: ${destinationId}, type: ${bestMatch.type})`
        );
      }
    } catch (autoError) {
    ...
    logger.info(
      `Found ${tours.length} tours in ${searchDestination}, returning ${toursToReturn.length} (hasMore: ${hasMore})`
    );
...
function findBestDestinationMatch(query, results) {
  ...
  logger.info(
    `Destination matching for "${query}": ${scored
      .slice(0, 3)
...
async function executeSearchFlights(input) {
  ...
  return {
    ...
    available: false,
    message:
      'I specialize in tours and experiences! Flight booking is coming soon. For now, I can help you find amazing tours and activities at your destination.',
...
async function executeSearchHotels(input) {
  ...
  return {
    ...
    available: false,
    message:
      'I specialize in tours and experiences! Hotel booking is coming soon. For now, I can help you find amazing tours and activities at your destination.',
    suggestion: `Would you like me to search for tours in ${destination} instead?`,
  };
}
...
async function executeGetDestinationInfo(input) {
  ...
  return {
    ...
    topics:
      topics.length > 0
        ? topics
        : ['overview', 'best_time_to_visit', 'neighborhoods', 'local_tips'],
