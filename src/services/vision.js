...
async function detectLandmarks(imageBase64) {
  ...
  try {
    ...
    logger.info(
      `Vision API: ${landmarks.length} landmarks, guesses: [${bestGuessLabels.join(', ')}]`
    );
...
{"identified":true,"confidence":"high","destination":{"name":"City","region":"Region","country":"Country","fullName":"City, Country"},"landmark":"FULL official name of landmark or null","reasoning":"One sentence"}`;
    ...
    logger.info(
      `Gemini identified: ${analysis.destination?.fullName || 'Unknown'} (${analysis.confidence})`
    );
...
async function identifyLocation(imageBase64, mediaType = 'image/jpeg') {
  ...
  if (visionResult.landmarks?.length > 0 && visionResult.landmarks[0].score > 0.7) {
    ...
    logger.info(
      `High-confidence landmark: ${topLandmark.name} (${Math.round(topLandmark.score * 100)}%)`
    );
    ...
    if (aiResult.success && aiResult.destination) {
      ...
      result.landmark =
        visionLandmark && visionLandmark.length > (aiLandmark?.length || 0)
          ? visionLandmark
          : aiLandmark || visionLandmark;
      result.confidence = 'high';
      result.reasoning = `Identified landmark: ${result.landmark}`;
      result.viatorDestinationId =
        aiResult.destination.viatorId || getDestinationId(aiResult.destination.name);
  ...
  if (aiResult.success && aiResult.identified && aiResult.destination) {
    ...
    logger.info(
      `Location identified in ${Date.now() - startTime}ms: ${result.destination?.fullName}`
    );
    return result;
  }
...
export {
  ...
  detectLandmarks,
  lookupPlace,
  analyzeImageWithGemini as analyzeImageWithAI,
};
